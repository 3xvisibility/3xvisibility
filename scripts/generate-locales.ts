...
// ── 2. Generate translations.ts (eager-bundled for instant language switch) ──
...
const loadersEntries = PRIMARY_CODES.map((c) => `  ${c}: async () => ({ default: ${c}Locale }),`).join("\n");
...
/**
 * All primary locales are eager-bundled so a language switch is
 * synchronous (no network, no lazy chunk). The extra gzip cost is
 * small compared to the ~300-800ms saved on first switch.
 */
