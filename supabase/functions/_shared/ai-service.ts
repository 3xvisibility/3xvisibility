...
// external-provider failure plus Lovable fallback can still return structured
// JSON before the platform timeout.
async function fetchWithTimeout(url: string, init: RequestInit, ms = 55_000): Promise<Response> {
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") {
      throw new Error("AI request timed out. Please try again with fewer rows or a shorter prompt.");
    }
    throw err;
  } finally {
    clearTimeout(timer);
