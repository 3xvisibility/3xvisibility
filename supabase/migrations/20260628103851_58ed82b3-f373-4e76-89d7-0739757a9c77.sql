...
DECLARE
  ...
  v_parse_line text;
  v_parse_values text[];
  v_parse_current text;
  v_parse_in_quotes boolean;
  v_parse_char text;
  v_parse_next text;
  v_parse_i integer;
  v_parse_len integer;
  ...
  WHILE v_i <= v_len LOOP
    ...
    IF (v_char = E'\n' OR v_char = E'\r') AND NOT v_in_quotes THEN
      IF btrim(v_record) <> '' THEN
        v_parse_line := v_record;
        v_parse_values := ARRAY[]::text[];
        v_parse_current := '';
        v_parse_in_quotes := false;
        v_parse_i := 1;
        v_parse_len := length(v_parse_line);

        IF v_records_seen = 0 THEN
          IF position(E'\t' in v_record) > 0 THEN
            v_delimiter := E'\t';
          ELSIF (length(v_record) - length(replace(v_record, ';', ''))) > (length(v_record) - length(replace(v_record, ',', ''))) THEN
            v_delimiter := ';';
          ELSIF (length(v_record) - length(replace(v_record, '|', ''))) > (length(v_record) - length(replace(v_record, ',', ''))) THEN
            v_delimiter := '|';
          ELSE
        ...
        WHILE v_parse_i <= v_parse_len LOOP
          v_parse_char := substr(v_parse_line, v_parse_i, 1);
          v_parse_next := CASE WHEN v_parse_i < v_parse_len THEN substr(v_parse_line, v_parse_i + 1, 1) ELSE '' END;

          IF v_parse_char = '"' THEN
            IF v_parse_in_quotes AND v_parse_next = '"' THEN
              v_parse_current := v_parse_current || '"';
              v_parse_i := v_parse_i + 2;
              CONTINUE;
            ELSE
              v_parse_in_quotes := NOT v_parse_in_quotes;
              v_parse_i := v_parse_i + 1;
              CONTINUE;
            END IF;
          END IF;

          IF v_parse_char = v_delimiter AND NOT v_parse_in_quotes THEN
            v_parse_values := array_append(v_parse_values, btrim(v_parse_current));
            v_parse_current := '';
            v_parse_i := v_parse_i + 1;
            CONTINUE;
          END IF;

          v_parse_current := v_parse_current || v_parse_char;
          v_parse_i := v_parse_i + 1;
        END LOOP;
        v_parse_values := array_append(v_parse_values, btrim(v_parse_current));

        IF v_records_seen = 0 THEN
          v_headers := v_parse_values;
        ELSE
          IF v_data_index >= v_start AND jsonb_array_length(v_rows) < v_take THEN
            v_values := v_parse_values;
            v_obj := '{}'::jsonb;
...
REVOKE ALL ON FUNCTION public.get_campaign_csv_window(uuid, integer, integer) FROM PUBLIC;
