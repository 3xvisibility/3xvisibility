CREATE OR REPLACE FUNCTION public.campaign_limit_for_plan(_plan text)
RETURNS integer
LANGUAGE sql
IMMUTABLE
AS $$
...
BEGIN
  ...
  v_limit := public.campaign_limit_for_plan(v_plan);
  ...
  IF v_limit >= 0 THEN
    ...
    IF v_count >= v_limit THEN
      RAISE EXCEPTION 'Plan limit reached: your % plan allows % campaign(s). Upgrade to add more.', v_plan, v_limit
