    ...
    try {
      const { data, error } = await supabase.functions.invoke("generate-template", {
        body: {
          ...
          existingContent: content,
          instruction:
            "Rewrite this HTML into a FULLY REUSABLE SEO template. Replace EVERY concrete/branded literal — including inside testimonials, reviews, stat blocks, contact blocks, footers, forms, and JSON-LD — with a {snake_case} placeholder. Nothing hard-coded should survive: company names, people names, street addresses, cities/states/countries, phone numbers, emails, URLs, prices, currencies, ratings, years, counts, service names. " +
            "CRITICAL CONTEXT — WHO IS WHO: The template belongs to the SITE OWNER (a business publishing this page). Testimonials/reviews are written BY CUSTOMERS ABOUT the site owner's business. So: (a) the business/brand/company being praised, its address, phone, email, website, and prices are the SITE OWNER — always use {business_name}, {company_name}, {brand_name}, {address}, {phone}, {email}, {website}, {price}. NEVER map these to {testimonial_company}. (b) Only the REVIEWER's own identity is testimonial-scoped: their name → {testimonial_name}, their job/role → {testimonial_role}, their city/location → {testimonial_location}, and the quote text → {testimonial_text}. Use {testimonial_company} ONLY if the reviewer's OWN employer is explicitly named separately from the business being reviewed — otherwise do not invent it. " +
            "Preferred vocabulary (use these when they fit): {service}, {service_name}, {service_type}, {quality}, {benefit}, {keyword}, {city}, {state}, {region}, {country}, {zip}, {address}, {business_name}, {brand_name}, {company_name}, {phone}, {email}, {website}, {price}, {currency}, {year}, {rating}, {stat_number}, {testimonial_text}, {testimonial_name}, {testimonial_role}, {testimonial_location}, {faq_question}, {faq_answer}, {cta_text}, {heading}, {sub_heading}, {description}. Use numbered suffixes like {testimonial_text_1}, {testimonial_name_1}, {testimonial_text_2}… for repeated review blocks so each reviewer is unique — but the business fields ({business_name}, {address}, {phone}, {price}) stay single, unnumbered, because they refer to the SAME site owner everywhere. " +
            "Hard rules: (1) NEVER change tags, classes, ids, inline styles, image URLs, SVG paths, scripts, or layout — only swap visible text nodes, alt attributes, meta content, aria-labels, and JSON-LD string values. (2) Every visible proper noun, phone, address, price, rating, and reviewer name MUST become a variable — no exceptions like 'Pixel Forge Studio', '$500', 'Ryan Johnson', 'California', or '123 Main St'. (3) Do not add or remove sections. (4) Return ONLY the final HTML, no explanations, no code fences.",
        },
      });
