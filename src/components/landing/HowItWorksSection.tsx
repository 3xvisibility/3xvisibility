...
export function HowItWorksSection() {
  ...
  return (
    <section id="how-it-works" className="py-20 md:py-28 relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_30%_at_50%_100%,hsl(217,91%,60%,0.06),transparent)] pointer-events-none" />
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>
          {steps.map((s, i) => (
            <motion.div key={s.titleKey} className="relative rounded-2xl border border-[hsl(217,91%,60%,0.1)] bg-[hsl(220,40%,8%)] p-6 text-center hover:border-[hsl(217,91%,60%,0.25)] transition-all duration-500 group" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } } }} whileHover={{ y: -4 }}>
              <div className="text-5xl font-black text-[hsl(217,91%,60%,0.05)] absolute top-3 right-4 select-none">{i + 1}</div>
              <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-[hsl(217,91%,60%,0.15)] to-[hsl(210,100%,70%,0.1)] border border-[hsl(217,91%,60%,0.2)] flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300">
                <s.icon className="h-5 w-5 text-[hsl(217,91%,68%)]" />
              </div>
