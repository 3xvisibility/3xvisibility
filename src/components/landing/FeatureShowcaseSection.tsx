...
export function FeatureShowcaseSection() {
  ...
  const showcases = [
    {
      ...
      visual: (
        <div className="rounded-xl border border-[hsl(217,91%,60%,0.12)] bg-[hsl(220,40%,8%)] p-4 space-y-2.5">
          {["keyword", "city", "title", "meta_desc"].map((col, i) => (
            <motion.div key={col} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 + i * 0.06 }} className="flex items-center gap-2">
              ...
              <ArrowRight className="h-3 w-3 text-[hsl(217,91%,60%,0.5)] shrink-0" />
              <div className="h-7 flex-1 rounded-lg bg-[hsl(217,91%,60%,0.08)] border border-[hsl(217,91%,60%,0.15)] flex items-center px-2 text-[10px] text-[hsl(217,91%,68%)] font-mono">{`{${col}}`}</div>
            </motion.div>
          ))}
        </div>
      ),
    },
    {
      ...
      visual: (
        <div className="rounded-xl border border-[hsl(217,91%,60%,0.12)] bg-[hsl(220,40%,8%)] overflow-hidden">
          ...
          <div className="p-4 space-y-1.5 font-mono text-[11px] leading-relaxed">
            <div className="text-[hsl(220,15%,45%)]">&lt;h1&gt;<span className="text-[hsl(217,91%,68%)] font-semibold">{"{title}"}</span>&lt;/h1&gt;</div>
            <div className="text-[hsl(220,15%,45%)]">&lt;meta name="desc" content="<span className="text-[hsl(217,91%,68%)] font-semibold">{"{meta_desc}"}</span>" /&gt;</div>
            <div className="text-[hsl(220,15%,45%)]">&lt;p&gt;Best in <span className="text-[hsl(217,91%,68%)] font-semibold">{"{city}"}</span>&lt;/p&gt;</div>
          </div>
        </div>
      ),
    },
        ...
        <div className="rounded-xl border border-[hsl(217,91%,60%,0.12)] bg-[hsl(220,40%,8%)] p-4 space-y-2.5">
          {[
            ...
            <motion.div key={page.title} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 + i * 0.08 }} className="flex items-center gap-2">
              <CheckCircle2 className={`h-3.5 w-3.5 shrink-0 ${page.status === "done" ? "text-[hsl(142,76%,50%)]" : "text-[hsl(217,91%,68%)] animate-pulse"}`} />
              <span className="text-[10px] font-mono text-[hsl(220,15%,55%)] flex-1 truncate">{page.title}</span>
              <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full ${page.status === "done" ? "bg-[hsl(142,76%,36%,0.1)] text-[hsl(142,76%,50%)]" : "bg-[hsl(217,91%,60%,0.1)] text-[hsl(217,91%,68%)]"}`}>
  ...
  return (
    <section className="py-20 md:py-28 relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_50%_50%,hsl(217,91%,60%,0.04),transparent)] pointer-events-none" />
              ...
              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14 items-center ${idx % 2 === 1 ? "lg:[direction:rtl] lg:*:[direction:ltr]" : ""}`}>
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-xs font-bold text-[hsl(217,91%,68%)] bg-[hsl(217,91%,60%,0.1)] border border-[hsl(217,91%,60%,0.2)] rounded-full px-3 py-1">{item.step}</span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.highlights.map((h) => (
                      <span key={h} className="text-[11px] font-medium text-[hsl(220,15%,60%)] bg-[hsl(220,35%,10%)] rounded-full px-3 py-1 border border-[hsl(217,91%,60%,0.1)]">{h}</span>
                    ))}
                  </div>
                </div>
                <div className="relative">
                  <div className="absolute -inset-4 bg-[radial-gradient(ellipse_at_center,hsl(217,91%,60%,0.06),transparent)] rounded-2xl blur-2xl pointer-events-none" />
