  ...
  return (
    <section id="integrations" className="py-20 md:py-28 relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none"
        style={{
          backgroundImage: `linear-gradient(hsl(258,88%,52%) 1px, transparent 1px), linear-gradient(90deg, hsl(258,88%,52%) 1px, transparent 1px)`,
          backgroundSize: "60px 60px",
        }}
      />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_50%_60%_at_50%_55%,hsl(258,88%,58%,0.08),transparent)] pointer-events-none" />
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        <div className="relative max-w-5xl mx-auto hidden md:block" style={{ height: 420 }}>
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1000 420" fill="none" preserveAspectRatio="xMidYMid meet">
            {[
              ...
              <motion.path key={i} d={p.d} stroke="hsl(258,88%,58%,0.15)" strokeWidth="1.5" fill="none" initial={{ pathLength: 0, opacity: 0 }} whileInView={{ pathLength: 1, opacity: 1 }} viewport={{ once: true }} transition={{ delay: p.delay, duration: 0.8, ease }} />
            ))}
          </svg>

          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
            <div className="absolute -inset-16 bg-[radial-gradient(circle,hsl(258,88%,58%,0.12),transparent_70%)] rounded-full blur-2xl" />
            <motion.div initial={{ opacity: 0, scaleX: 0 }} whileInView={{ opacity: 1, scaleX: 1 }} viewport={{ once: true }} transition={{ delay: 0.8, duration: 0.6, ease }} className="absolute top-1/2 -translate-y-1/2 right-full mr-0 w-[200px] h-[2px] origin-right">
              <div className="h-full bg-gradient-to-l from-[hsl(258,88%,58%,0.6)] via-[hsl(258,88%,58%,0.15)] to-transparent" />
              <div className="absolute inset-y-0 right-0 w-8 h-[6px] -translate-y-[2px] bg-[hsl(258,88%,58%,0.4)] blur-sm" />
            </motion.div>
            <motion.div initial={{ opacity: 0, scaleX: 0 }} whileInView={{ opacity: 1, scaleX: 1 }} viewport={{ once: true }} transition={{ delay: 0.8, duration: 0.6, ease }} className="absolute top-1/2 -translate-y-1/2 left-full ml-0 w-[200px] h-[2px] origin-left">
              <div className="h-full bg-gradient-to-r from-[hsl(258,88%,58%,0.6)] via-[hsl(258,88%,58%,0.15)] to-transparent" />
              <div className="absolute inset-y-0 left-0 w-8 h-[6px] -translate-y-[2px] bg-[hsl(258,88%,58%,0.4)] blur-sm" />
            </motion.div>
            <motion.div animate={{ scale: [1, 1.35, 1], opacity: [0.25, 0, 0.25] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} className="absolute -inset-5 rounded-[22px] border border-[hsl(258,88%,58%,0.2)]" />
            <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.15, 0, 0.15] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 0.6 }} className="absolute -inset-3 rounded-[20px] border border-[hsl(258,88%,58%,0.15)]" />
            <motion.div initial={{ opacity: 0, scale: 0.6 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.3, ease }} className="relative h-[130px] w-[130px] rounded-2xl bg-gradient-to-b from-[hsl(258,88%,52%)] to-[hsl(258,88%,52%)] flex items-center justify-center shadow-[0_0_80px_hsl(258,88%,58%,0.35),0_0_30px_hsl(258,88%,58%,0.2)] border border-[hsl(258,88%,58%,0.2)]">
              ...
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-3/4 h-4 bg-[hsl(258,88%,58%,0.3)] blur-lg rounded-full" />
            </motion.div>
          </div>
            ...
            return (
              <motion.div key={item.name} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 + i * 0.1, duration: 0.5, ease }} className="absolute" style={{ top: positions[i].top, left: positions[i].left }}>
                <div className="flex flex-col items-center gap-1.5">
                  <div className="relative h-[52px] w-[52px] rounded-xl bg-[hsl(250,30%,97%)] border border-[hsl(250,30%,92%)] flex items-center justify-center shadow-lg hover:border-[hsl(258,88%,58%,0.3)] hover:shadow-[0_0_20px_hsl(258,88%,58%,0.1)] transition-all duration-300 cursor-pointer group">
                    <item.icon className="h-5 w-5 text-[hsl(220,12%,42%)] group-hover:text-[hsl(258,88%,52%)] transition-colors" />
                  ...
                  <span className="text-[10px] font-medium text-[hsl(220,12%,42%)]">{item.name}</span>
                </div>
              </motion.div>
            );
          })}
          ...
          {[...leftItems, ...rightItems].map((item, i) => (
            <motion.div key={item.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 + i * 0.08, duration: 0.4, ease }} className="flex flex-col items-center gap-2">
              <div className="relative h-12 w-12 rounded-xl bg-[hsl(250,30%,97%)] border border-[hsl(250,30%,92%)] flex items-center justify-center">
                <item.icon className="h-5 w-5 text-[hsl(258,88%,52%)]" />
                {"soon" in item && item.soon && (
                  <span className="absolute -top-2 -right-2 rounded-full bg-amber-500/15 text-amber-500 border border-amber-500/30 text-[8px] font-semibold px-1.5 py-0.5 leading-none">{t("common.soon")}</span>
                )}
              </div>
        ...
        <ScrollReveal>
          <div className="mt-16 md:mt-20 max-w-2xl mx-auto">
            <div className="relative rounded-2xl border border-[hsl(258,88%,58%,0.18)] bg-[hsl(250,30%,98%)] p-6 md:p-8 text-center overflow-hidden">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,hsl(258,88%,58%,0.1),transparent_60%)] pointer-events-none" />
              <div className="relative z-10">
                <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[hsl(258,88%,58%,0.12)] border border-[hsl(258,88%,58%,0.25)]">
                  <Plug className="h-5 w-5 text-[hsl(258,88%,52%)]" />
                </div>
