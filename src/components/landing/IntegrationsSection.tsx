  ...
  return (
    <section id="integrations" className="py-20 md:py-28 relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none"
        style={{
          backgroundImage: `linear-gradient(hsl(96,90%,45%) 1px, transparent 1px), linear-gradient(90deg, hsl(96,90%,45%) 1px, transparent 1px)`,
          backgroundSize: "60px 60px",
        }}
      />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_50%_60%_at_50%_55%,hsl(96,90%,45%,0.08),transparent)] pointer-events-none" />
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <ScrollReveal>
          <div className="text-center mb-16 md:mb-20">
            ...
            <h2 className="font-editorial text-4xl md:text-5xl lg:text-[3.4rem]">
        ...
        <div className="relative max-w-5xl mx-auto hidden md:block" style={{ height: 420 }}>
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1000 420" fill="none" preserveAspectRatio="xMidYMid meet">
            {[
              ...
              <motion.path key={i} d={p.d} stroke="hsl(96,90%,45%,0.15)" strokeWidth="1.5" fill="none" initial={{ pathLength: 0, opacity: 0 }} whileInView={{ pathLength: 1, opacity: 1 }} viewport={{ once: true }} transition={{ delay: p.delay, duration: 0.8, ease }} />
            ))}
          </svg>

          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
            <div className="absolute -inset-16 bg-[radial-gradient(circle,hsl(96,90%,45%,0.12),transparent_70%)] rounded-full blur-2xl" />
            <motion.div initial={{ opacity: 0, scaleX: 0 }} whileInView={{ opacity: 1, scaleX: 1 }} viewport={{ once: true }} transition={{ delay: 0.8, duration: 0.6, ease }} className="absolute top-1/2 -translate-y-1/2 right-full mr-0 w-[200px] h-[2px] origin-right">
              <div className="h-full bg-gradient-to-l from-[hsl(96,90%,45%,0.6)] via-[hsl(96,90%,45%,0.15)] to-transparent" />
              <div className="absolute inset-y-0 right-0 w-8 h-[6px] -translate-y-[2px] bg-[hsl(96,90%,45%,0.4)] blur-sm" />
            </motion.div>
            <motion.div initial={{ opacity: 0, scaleX: 0 }} whileInView={{ opacity: 1, scaleX: 1 }} viewport={{ once: true }} transition={{ delay: 0.8, duration: 0.6, ease }} className="absolute top-1/2 -translate-y-1/2 left-full ml-0 w-[200px] h-[2px] origin-left">
              <div className="h-full bg-gradient-to-r from-[hsl(96,90%,45%,0.6)] via-[hsl(96,90%,45%,0.15)] to-transparent" />
              <div className="absolute inset-y-0 left-0 w-8 h-[6px] -translate-y-[2px] bg-[hsl(96,90%,45%,0.4)] blur-sm" />
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.6 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.3, ease }} className="relative h-[130px] w-[130px] rounded-2xl bg-gradient-to-b from-[hsl(96,90%,38%)] to-[hsl(96,70%,28%)] flex items-center justify-center shadow-[0_0_80px_hsl(96,90%,45%,0.35),0_0_30px_hsl(96,90%,45%,0.2)] border border-[hsl(96,80%,62%,0.2)]">
              ...
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-3/4 h-4 bg-[hsl(96,90%,45%,0.3)] blur-lg rounded-full" />
            </motion.div>
          </div>
