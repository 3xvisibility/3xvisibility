...
export function ShopifyCredentialFields({
  ...
  const tokenOk = accessToken.startsWith("shpat_") && accessToken.length > 10;
  const tokenError = accessToken && !tokenOk ? "Token should start with shpat_" : null;
    ...
    <div className="space-y-4">
      <Alert className="bg-primary/5 border-primary/20">
        ...
        <AlertDescription className="text-xs leading-relaxed">
          <strong>Admin API Access Token</strong> — Go to your Shopify Admin → Settings → Apps
          → Develop apps → Create/select an app → API credentials → Admin API access token.
        </AlertDescription>
      </Alert>
      ...
      <div>
        ...
        {tokenError ? (
          ...
          <p className="text-[11px] text-muted-foreground mt-1">
            Found in Develop apps → your app → API credentials → <strong>Admin API access token</strong>
          </p>
        )}
      </div>
    </div>
