...
export default function IndexingPage() {
  ...
  const { data: websites = [] } = useQuery({
    ...
    enabled: !!wsId,
    queryFn: async () => {
      const { data, error } = await supabase.from("websites").select("id, user_id, name, url, type, status, last_sync, created_at, updated_at, google_indexing_enabled, workspace_id, language, language_locked, shop_details, inline_assets_fallback").eq("workspace_id", wsId!).order("name");
