...
function detectSpecialWidget(node: HtmlNode): ElementorElement | null {
  ...
  if(process.env.DBG) node.children.forEach((c:any)=>console.error("CHILD", JSON.stringify(c.tag), JSON.stringify(c.attrs&&c.attrs.class), "txt="+JSON.stringify(textContent(c)), "hc="+(c.tag?hasClass(c,"stat","counter"):null)));
